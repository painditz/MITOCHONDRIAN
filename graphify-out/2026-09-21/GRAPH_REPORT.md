# Graph Report - MICROSOFT WINNER  (2026-09-21)

## Corpus Check
- 253 files · ~508,239 words
- Verdict: corpus is large enough that graph structure adds value.
- Unclassified: 30 file(s) not represented in the graph (top: .toml 18, (none) 6, .css 3)

## Summary
- 1880 nodes · 2411 edges · 160 communities (118 shown, 42 thin omitted)
...
1. `react` - 44 edges
2. `NormalizedAlert` - 24 edges
3. `generate_alerts()` - 19 edges
4. `Code Review and Quality` - 19 edges
5. `correlate_normalized_alerts()` - 18 edges
6. `Security and Hardening` - 17 edges
7. `RawAlert` - 16 edges
8. `score_normalized_alerts()` - 15 edges
9. `Incident` - 15 edges
10. `calculate_mttt_metrics()` - 15 edges
...
- `enrich_incidents_with_briefs()` --uses--> `Incident`  [INFERRED]
  backend/ai_briefer.py → backend/models.py
- `generate_3000_alerts()` --uses--> `RawAlert`  [INFERRED]
  backend/ingestion.py → backend/models.py
...
### Community 9 - "DigitalWater.jsx"
Cohesion: 0.33
Nodes (3): DigitalWater(), @react-three/fiber, three
...
Cohesion: 0.15
Nodes (24): correlate_normalized_alerts(), Determines which alerts are related and groups them into unified incidents.…, generate_3000_alerts(), Default demonstration entry point producing exactly 3,000 synthetic alerts with…, map_alerts_to_mitre(), Maps alert evidence to MITRE ATT&CK techniques ONLY when concrete evidence…, MitreTechnique, NormalizedAlert (+16 more)
...
### Community 91 - "app.py"
Cohesion: 0.09
Nodes (47): AnalystFeedbackRequest, AnalystReviewRequest, get_mttt_metrics(), get_soc_overview(), initialize_pipeline(), BaseModel, Human-in-the-Loop Review Controls: Confirm, Reject, Modify, Add Analyst Note,…, Records whether the human SOC analyst agrees with the AI-generated brief.… (+39 more)
...
### Community 139 - "IncidentIntelligence.jsx"
Cohesion: 0.21
Nodes (9): ArchitecturePipeline(), STAGES, PlaneTopology(), IncidentIntelligence(), IncidentSummaryStrip(), AiMlEvaluationLab(), Reveal(), SectionHeader() (+1 more)

### Community 140 - "verify_endpoints.py"
Cohesion: 0.47
Nodes (5): http_get(), http_post(), run_verification(), time, urllib_request

### Community 141 - "_build_scaled_attack_scenarios"
Cohesion: 0.43
...
### Community 143 - "get"
Cohesion: 0.22
Nodes (9): get_incident_detail(), get_incident_queue(), get_ml_metrics(), get_raw_alerts(), Incident Queue: View B required by Problem #25, Incident Details: View C required by Problem #25, Model Evaluation Metrics: View required by Section 12 of Problem #25 upgrade.…, Paginated Raw Alerts for Inspection (+1 more)

### Community 144 - "run-evals-test.js"
Cohesion: 0.14
...
### Community 147 - "MitreIntelligence.jsx"
Cohesion: 0.26
Nodes (10): IncidentDetailModal(), parseRiskFactors(), MitreDrawer(), BASE_TECHNIQUES, AuditStatusBadge(), CriticalityBadge(), PriorityBadge(), RawSeverityBadge() (+2 more)

### Community 148 - "IncidentMapPanel.jsx"
Cohesion: 0.22
Nodes (10): BASE_XY_COORDINATES, CurvedGraphRelationships(), getIncident3DPosition(), getPriorityTheme(), hashString(), IncidentGraphNode(), IncidentMapPanel(), ParallaxCameraController() (+2 more)
...
Cohesion: 0.21
Nodes (8): App(), MitreIntelligence(), Navigation(), AmbientAtmosphere(), RawTelemetryLake(), rootElement, client_src_styles_index, ref_react_dom_client
...
## Knowledge Gaps
- **1073 isolated node(s):** `$schema`, `name`, `description`, `name`, `url` (+1068 more)
