...
1. `react` - 44 edges
2. `NormalizedAlert` - 33 edges
3. `generate_alerts()` - 19 edges
4. `Code Review and Quality` - 19 edges
5. `correlate_normalized_alerts()` - 18 edges
6. `Incident` - 17 edges
7. `Security and Hardening` - 17 edges
8. `build_authoritative_graph()` - 16 edges
9. `RawAlert` - 16 edges
10. `score_normalized_alerts()` - 15 edges
...
- `get_correlation_graph()` --references--> `get()`  [EXTRACTED]
  backend/app.py → scratch/verify_e2e_live.py
...
### Community 9 - "test_dataset_upgrade.py"
Cohesion: 0.14
Nodes (17): argparse, DatasetValidationReport, Dataset Validator for Problem Statement #25 ("3,000 Alerts, One Analyst").…, validate_dataset(), Synthetic Training Dataset Generator for Microsoft Problem Statement #25.…, RawAlert, normalize_alert(), normalize_batch() (+9 more)
...
Cohesion: 0.06
Nodes (35): 1. Preserve Behavior Exactly, 2. Follow Project Conventions, 3. Prefer Clarity Over Cleverness, 4. Maintain Balance, 5. Scope to What Changed, Code Simplification, Common Rationalizations, Language-Specific Guidance (+27 more)
...
Cohesion: 0.16
Nodes (21): correlate_normalized_alerts(), Determines which alerts are related and groups them into unified incidents.…, map_alerts_to_mitre(), Maps alert evidence to MITRE ATT&CK techniques ONLY when concrete evidence…, MitreTechnique, ShiftBrief, calculate_incident_priority(), Computes an explainable Risk Score (0 - 100) and Priority Tier. Crucial… (+13 more)
...
### Community 91 - "app.py"
Cohesion: 0.08
Nodes (45): AnalystFeedbackRequest, AnalystReviewRequest, get_mttt_metrics(), get_soc_overview(), initialize_pipeline(), BaseModel, Human-in-the-Loop Review Controls: Confirm, Reject, Modify, Add Analyst Note,…, Records whether the human SOC analyst agrees with the AI-generated brief.… (+37 more)
...
Cohesion: 0.09
Nodes (24): _build_attack_scenarios(), _build_scaled_attack_scenarios(), generate_3000_alerts(), generate_alerts(), Any, datetime, Default demonstration entry point producing exactly 3,000 synthetic alerts with…, Synthetic Security Alert Generator for Microsoft Problem Statement #25 ("3,000… (+16 more)

### Community 135 - "Incident"
Cohesion: 0.31
...
Cohesion: 0.23
Nodes (20): get_correlation_graph(), Authoritative Correlation Graph Contract: Returns the real graph nodes and…, build_authoritative_graph(), extract_incident_entities(), parse_iso(), Any, datetime, Generates the authoritative correlation graph structure derived directly from… (+12 more)
...
Cohesion: 0.06
Nodes (30): dependencies, canvas-confetti, lucide-react, react, react-dom, @react-three/drei, @react-three/fiber, three (+22 more)

### Community 147 - "IncidentIntelligence.jsx"
Cohesion: 0.16
Nodes (18): IncidentDetailModal(), parseRiskFactors(), IncidentIntelligence(), IncidentSummaryStrip(), MitreDrawer(), BASE_TECHNIQUES, MitreIntelligence(), AiMlEvaluationLab() (+10 more)

### Community 148 - "IncidentMapPanel.jsx"
Cohesion: 0.15
Nodes (14): BASE_XY_COORDINATES, CurvedGraphRelationships(), findCorrelationEvidence(), getIncident3DPosition(), getIncidentEntities(), getPriorityTheme(), hashString(), IncidentGraphNode() (+6 more)
...
Cohesion: 0.17
Nodes (9): App(), ArchitecturePipeline(), STAGES, PlaneTopology(), Navigation(), DigitalWater(), rootElement, client_src_styles_index (+1 more)
...
## Knowledge Gaps
- **1072 isolated node(s):** `$schema`, `name`, `description`, `name`, `url` (+1067 more)
